...
static int __init dmar_x2apic_optout(void)
{
	...
	if (!dmar || no_x2apic_optout)
		return 0;
	//返回值表示系统是否使用X2APIC功能（1表示不使用，0表示使用）
...
static int __init intel_enable_irq_remapping(void)
{
	...
	for_each_iommu(iommu, drhd) {
		//如果硬件单元的queued invalidation还没有初始化，
		//则清掉之前的fault，并且disable IRQ remapping和queued invalidation。
	 ...
	 */
	 //如果硬件支持IRQ remapping并且系统支持x2APIC模式，
	 //如果硬件不支持x2APIC模式，就失败。
...
//intel的Interrupt Remapping初始化相关部分
struct irq_remap_ops intel_irq_remap_ops = {
//被irq_remapping_prepare调用

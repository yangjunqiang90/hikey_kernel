...
{//有点像
	struct pci_dev *dev;
//过滤窗口决定了访问的方向。例如：如果cpu一侧要经过
//pci bridge访问pci总线，则它的地址必须要落在这个pci桥的过
//滤窗口内才可以通过。另外，pci bridge下游的pci bus要访问
//cpu侧。则地址必须要落在过滤窗口外才可以。
	pci_read_bridge_bases(b);//进来吧
...
void pcibios_scan_root(int busnum)
{
	...
	//给总线分配资源
	x86_pci_root_bus_resources(busnum, &resources);//进来吧
	printk(KERN_DEBUG "PCI: Probing PCI hardware (bus %02x)\n", busnum);
	//以前是pci_scan_bus_parented，真正的PCI设备探测
	...
	//遍历总线之后。如果总线遍历成功,就会转入
	//pci_bus_add_devices().
	pci_bus_add_devices(bus);//进来 
}
